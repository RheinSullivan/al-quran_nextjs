    ...
    <header className="fixed top-0 left-0 w-full bg-transparent py-3 px-6 flex justify-between items-center shadow">
      ...
      <div className="flex items-center space-x-3 pr-14">
        ...
        <div className="relative">
          <button onClick={() => setSubmenuOpen(!submenuOpen)} className="bg-transparent text-white px-4 py-2 cursor-pointer flex items-center">
          ...
          {submenuOpen && (
            <ul className="absolute right-0 mt-2 w-40 bg-transparent bg-gray-200 rounded-md text-black overflow-hidden z-10">
              ...
              <li className="px-4 py-2 hover:bg-dark2 hover:text-white cursor-pointer" onClick={() => handleLanguageChange({ target: { value: "ind" } } as React.ChangeEvent<HTMLSelectElement>)}>
                Indonesia
              </li>
            </ul>
          )}
