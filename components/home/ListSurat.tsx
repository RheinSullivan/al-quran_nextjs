...
export default function ListSurat() {
  ...
  return (
    <section className="pt-32 pb-10 text-white" id="al-qur'an">
      <div className="grid grid-cols-1 pl-14 mx-6 md:mx-20 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {allSurah.slice(0, page * itemsPerPage).map((surah) => (
          <div
            key={surah.nomor}
            className="p-4 bg-dark border border-gray-600 rounded-xl flex flex-col group"
          >
            ...
            <Link href={`/surat/${surah.nomor}`} className="cursor-pointer">
              <h1 className="text-lg md:text-xl font-bold pb-1 italic group-hover:text-green-500">
                {surah.namaLatin}
              </h1>
            </Link>
      ...
      <div className="flex justify-center md:text-xl pl-14 md:pl-0 mt-10">
        {loading ? (
          ...
          <button
            onClick={loadMore}
            className="bg-green-600 text-white px-4 py-2 hover:bg-gray-200 hover:text-green-600 rounded-md"
          >
