...
export default function Header({
  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full py-3 md:px-6 z-10 flex justify-between items-center transition-all duration-300 ${
        scrolled ? "bg-dark/50 backdrop-blur" : "bg-transparent"
      }`}
    >
      <div className="flex items-center pl-20 md:pl-28">
        <Image
          src={Logo}
          alt="Logo Al-Qur'an"
          className="w-[45px] md:w-[70px] mr-3"
        />
        <div className="text-white">
          <h1 className="text-sm md:text-xl font-semibold">
            Al-Qur&apos;an (Beta version)
          </h1>
      ...
      <div className="flex items-center space-x-3 pr-2 md:pr-14">
        <a
          href="#"
          className="hidden md:flex bg-gray-200 text-black px-4 py-2 rounded-md hover:bg-green-600 hover:text-white"
        >
          {language === "en"
        ...
        <div className="relative" ref={menuRef}>
          <button
            ...
            className="bg-transparent text-white px-4 py-2 cursor-pointer flex items-center text-xs md:text-sm"
          >
            {language === "en" ? "English" : "Indonesia"}
            <BsChevronDown
              className={`ml-2 transform duration-300 ${
